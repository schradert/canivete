...
in {
  options.canivete.meta = {
    ...
    people = can.submodule "people in the organization" ({config, ...}: {
      options.users = can.attrs.submodule "all of the users to create configurations for" ({name, ...}: {
        options.name = can.str "name of the user to default to in all contexts" {default = name;};
